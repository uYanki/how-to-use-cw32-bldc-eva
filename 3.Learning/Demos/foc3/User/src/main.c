...
float theta_e = 0.0;                // 电机电角度
float id      = 0.0;                // d轴电流
float iq      = 0.0;                // q轴电流
float ia = 0.0, ib = 0.0;           // 三相电流
float va = 0.0, vb = 0.0;           // 三相电压
float vd = 0.0, vq = 0.0;           // d轴和q轴电压
float v_alpha = 0.0, v_beta = 0.0;  // alpha-beta坐标系下的电压
float i_alpha = 0.0, i_beta = 0.0;  // alpha-beta坐标系下的电流

// 控制器变量
float e = 0.0, e_prev = 0.0;  // 当前误差和上一时刻误差
...
// FOC算法
void FOC(float target_speed, float current_speed, float target_torque, float current_torque)
{
    // 计算电机角度
    theta += current_speed * Ts;

    // 计算d轴和q轴电流
    id = target_torque / Ld;
    iq = (Ke * target_speed + target_torque / Km) / Lq;
    ...
    // 计算控制器输出电压
    e = target_speed - current_speed;                       // 当前误差
    integral += e * Ts;                                     // 积分项
    derivative = (e - e_prev) / Ts;                         // 微分项
    vq         = Kp * e + Ki * integral + Kd * derivative;  // 计算q轴电压
    vd         = -Ld / Lq * vq + Ke / Lq * target_speed;    // 计算d轴电压
    v_alpha    = vd * cos(theta) + vq * sin(theta);         // alpha坐标系下的电压
    v_beta     = -vd * sin(theta) + vq * cos(theta);        // beta坐标系下的电压
    ...
    ATIM_SetCompare1A((va + 6) / 12.f * 100);  // U
    ATIM_SetCompare2A((vb + 6) / 12.f * 100);  // V
    ATIM_SetCompare3A((vc + 6) / 12.f * 100);  // W
}
...
void BTIM1_IRQCallBack()
{
    ...
    if (nDelta > 0) {
        ...
        rpm = nDelta * (60000.f / CONFIG_DEBUG_TICKER_PERIOD_N_MS) / 12;
        // println("rpm:%d", rpm);
    }

    nHallLast = nHallCur;

    FOC(1000, rpm, 400, 400);
}
...
{
#define VBUS 12
#define VMIN (VBUS * 0.1)
#define VMAX (VBUS * 0.9)
#define VREF ((VMIN + VMAX) / 2)
    ...
    // clark
    Ualpha = Ua;
    Ubeta  = INVSQRT3 * (2 * Ub + Ua);

    // park
    Ud = Ubeta * sinf(theta) + Ualpha * cosf(theta);
    Uq = Ubeta * cosf(theta) - Ualpha * sinf(theta);
    ...
    theta = MOTOR_POLE_PAIRS * (DEG2RAD * (nCurDeg));  // 电角度 = 机械角度 * 电机极对数

    // ipark
    Ualpha = Ud * cosf(theta) - Uq * sinf(theta);
    Ubeta  = Ud * sinf(theta) + Uq * cosf(theta);
    ...
    Ua = CLAMP(Ua, VMIN, VMAX);
    Ub = CLAMP(Ub, VMIN, VMAX);
    Uc = CLAMP(Uc, VMIN, VMAX);

    f32 duty_a = Ua / VBUS * 100;
    f32 duty_b = Ub / VBUS * 100;
    f32 duty_c = Uc / VBUS * 100;

    // println("%f,%f,%f", duty_a, duty_b, duty_c);

    ATIM_SetCompare1A(duty_a);  // U
    ATIM_SetCompare2A(duty_b);  // V
    ATIM_SetCompare3A(duty_c);  // W
    // println("%f,%f,%f", Ua, Ub, Uc);
}
...
static void PWM_Configuration(void)
{
    ...
    }
    {
        ...
        GPIO_Init(CW_GPIOA, &GPIO_InitStruct);
        GPIO_WritePin(CW_GPIOB, GPIO_InitStruct.Pins, GPIO_Pin_RESET);   // OFF
    ...
    }
    {
        ...
        // ATIM_OC4Init(ENABLE, ATIM_OC_IT_UP_COUNTER, ENABLE, ENABLE, DISABLE);  // for ADc

        // ATIM_CH1Config(ATIM_CHxA_CIE | ATIM_CHxB_CIE, ENABLE);
        // ATIM_CH2Config(ATIM_CHxA_CIE | ATIM_CHxB_CIE, ENABLE);
        // ATIM_CH3Config(ATIM_CHxA_CIE | ATIM_CHxB_CIE, ENABLE);
...
{
    if (ATIM_GetITStatus(ATIM_IT_OVF)) { ATIM_ClearITPendingBit(ATIM_IT_OVF); }
    if (ATIM_GetITStatus(ATIM_IT_C1AF)) { ATIM_ClearITPendingBit(ATIM_IT_C1AF); }
    if (ATIM_GetITStatus(ATIM_IT_C1BF)) { ATIM_ClearITPendingBit(ATIM_IT_C1BF); }
}
